...
import { useCart,useUpdatePaymentSession,useCompleteCart,useSetPaymentSession } from "medusa-react"
...
const InjectableCardFormRazorpay = ({ session }) => {
  ...
  /*const {
    cart,
    actions: { completeCheckout, setPaymentSession,updatePaymentSession },
  } = useCart()*/
  ...
  const completeOrder = async (response) => {
    ...
    const cart = await useSetPaymentSession("razorpay")
    const router = useRouter()
    if (!cart) {
      setProcessing(false)
      return
    }
    const cart_updated = await useUpdatePaymentSession(cart.id,"razorpay",response)
    ...
    const order = await useCompleteCart(cart_updated.id)
